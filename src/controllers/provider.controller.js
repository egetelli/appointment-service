...
class ProviderController {
  async getProviders(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Veriler getirilemedi." });
    }
  }
  ...
  async assignService(req, res) {
    try {
      ...
      const data = await providerService.assignServiceToProvider(
        req.params.id,
        service_id,
        custom_price,
      );
  ...
  // Provider'ın sunduğu hizmetleri getir
  async getProviderServices(req, res) {
    try {
      const data = await providerService.getProviderServices(req.params.id);
      res.json({ success: true, data });
    } catch (error) {
      res
        .status(500)
        .json({ success: false, message: "Hizmetler getirilemedi." });
    }
  }
}
